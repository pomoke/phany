...
impl<Handle, Message> Viewer<Handle, Message> {
    ...
    ///
    /// Default is `16.0`, in times of image size.
    ...
    ///
    /// Default is `0.25`, in ratio of viewport size.
    ...
    /// Handler for scaling.
    pub fn on_scale(mut self, f: impl Fn(f32) -> Message + 'static) -> Self {
        {
            self.scale_handler = Some(Box::new(f));
            self
        }
    }

    /// Handler for moving.
    pub fn on_move(mut self, f: impl Fn(Vector) -> Message + 'static) -> Self {
        {
    ...
    Handle: Clone + Hash,
{
    ...
    fn state(&self) -> tree::State {
        let state = State {
            ...
            current_offset: self.position.unwrap_or(Default::default()),
            ..Default::default()
        };
                ...
                match delta {
                    mouse::ScrollDelta::Lines { y, .. } | mouse::ScrollDelta::Pixels { y, .. } => {
                            ...
                            || y > 0.0 && previous_scale < self.max_scale
                        {
                            ...
                            if let Some(ref handler) = self.scale_handler {
                                _shell.publish(handler(state.scale));
                            }
                            if let Some(ref handler) = self.move_handler {
                                _shell.publish(handler(state.current_offset));
                            }
                        }
                    }
                }
